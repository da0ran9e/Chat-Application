...
void execute_login_query(PGconn *conn, const char *username, const char *password) {
    ...
    printf("Login Authorization: %d\n", isAuthorized);

    // Free the result
    PQclear(result);
}

void execute_register_query(PGconn *conn, const char *username, const char *password) {
